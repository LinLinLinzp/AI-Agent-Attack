"""V165: safety floor -- exact V100 replica (92.25 LB, best seen).

Kept byte-identical to V100 so the inject-led race->bank->fill framework serves
as the deterministic ~92 floor under V162 (native injection) / V163 (reasoning
injection) / V164 (private probes). Competition sandbox only."""
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V165] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f(raw=%.2f/%.1f) banked=%d returned=%d "
