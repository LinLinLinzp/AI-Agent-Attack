"""V123 minimal templates. Blog Part 5 §7: top engine's lever 4 is
"hops-1 fill throughput lever". Each template's probe costs time; fewer templates
means less probe overhead, more fill time.
V121 routed 4 gpt templates + 2 gemma templates. V123 cuts to 2 gpt (bare fallback
+ 1 collapse) and 1 gemma (bare only). PROBE_REPS 3->2, RACE_EXTRA_REPS 2->1.
factor=3.0, FILL_HOPS=1, per-model routing preserved. Competition sandbox only."""
...
# V123 minimal: only 2 gpt templates (bare fallback + 1 collapse),
# 1 gemma template (bare only). Less probe overhead -> more fill time.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V123] model=%s selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f(raw=%.2f/%.1f) banked=%d returned=%d "
