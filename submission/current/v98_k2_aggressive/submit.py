"""V98 K1 volume maximizer. Based on v62 (92.160 LB) with three key improvements:
1. FILL_HOPS=1: fill at 1-hop (exfil fires at hop 0), faster search → more candidates
2. REPLAY_CORRECTION_FACTOR=2.0: gateway replay is 6-11x faster than search, but
   external 86.175 reference uses 1.03. Factor 2.0 is a conservative compromise.
3. eval_predicates + manual_exfil dual check (from 88.515 reference).
v62 params retained: MARGIN_S=45, MARGIN_MULT=1.20, K1_REPLAY_SAFE=0.99, 3-leader race.
...
K1_REPLAY_SAFE = 0.99
# Gateway replay latency is 6-11x lower than search trial latency
# (K1 gpt: search=10s vs replay=0.93s; K1 gemma: search=6s vs replay=1.04s).
# But external 86.175 reference uses REPLAY_COST_FACTOR=1.03 (only +3%).
# Factor 2.0 is a conservative compromise between the two data points.
...
# Same 5 templates as v62 (92.160 LB). Mix of special-token and natural-language.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V98] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f(raw=%.2f/%.1f) banked=%d returned=%d "
