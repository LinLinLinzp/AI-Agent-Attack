"""V71 diagnostic telemetry engine.
Exact V62 source with additional stderr telemetry for per-template statistics,
replay cost breakdown, and timing data. No functional change.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        _search_time = _elapsed_total  # total wall time of the search phase
        _per_template_stats = []
        ...
        # Summary with detailed telemetry
        print(
            "[V71] model=%s selected=%s(k%d) score=%.2f cost=%.3fs fill_unit=%.2fs "
