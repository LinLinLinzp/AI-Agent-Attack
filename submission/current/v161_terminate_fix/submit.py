"""V161: V156 terminate-injection with the FILL hops bug FIXED.

V156 scored 0.44 because fill used hops=1 (a speed leftover from V100) while
terminate templates require min_keep=2 posts -- hops=1 only permits ONE tool
call, so every fill trial fell short and nearly zero candidates were produced.
The terminate mechanism itself was never actually tested.

Fix: fill uses max_tool_hops (8) so a terminate template can really fire 2 posts
across turns (turn1 post1 -> turn2 post2). Note the generation env IS the real
remote model + public guardrail (gateway Phase 1 uses RemoteAgent), so a 2-post
race win is directly indicative of public-row behavior -- but private still
replay-checks separately.

Still hedged by inj_data_first (single-post floor) if no terminate variant lands
2 posts."""
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V161] selected=%s k=%d banked=%d returned=%d fill=%d/%d pool=%s | %s"
            % (
