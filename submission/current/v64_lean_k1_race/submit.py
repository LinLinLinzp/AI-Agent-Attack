"""V64 lean K1 race engine.
Keeps V62's K1-only exact validation and replay ledger, but races only the top
two one-shot leaders before fill to spend less search budget on exploration.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[v64_lean_k1_race] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f banked=%d returned=%d "
