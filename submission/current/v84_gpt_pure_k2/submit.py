"""V84 GPT pure K2 channel.
Gemma uses K1 templates (same as V62), GPT uses ONLY K2 templates (no K1 at all for GPT).
This gives GPT's candidates 34 raw each (2 http.post calls) instead of 18 raw.
...
K1_REPLAY_SAFE = 0.95  # Gemma
K1_GPT_REPLAY_SAFE = 0.95  # GPT (applied to K2 templates)
...
# K2 templates: each makes 2 http.post calls in one message (34 raw per fired trial).
# Used as GPT's primary templates (no K1 for GPT).
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V84] model=%s selected=%s(k%d) score=%.2f cost=%.3fs fill_unit=%.2fs "
