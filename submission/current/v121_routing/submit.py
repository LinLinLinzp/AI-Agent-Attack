"""V121 per-model routing. Blog Part 5 §2: run() is called per model, so
a selector scores templates by raw-per-second and picks the best ONCE PER MODEL.
gpt_oss reasons -> collapse templates (control-token skip CoT, 24s->10s).
gemma non-reasoning -> plain templates (control tokens are noise to gemma).
V100 gave both models the same 5 templates; gemma measured collapse as slow.
This version routes collapse to gpt, plain to gemma. Competition sandbox only."""
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V121] model=%s selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f(raw=%.2f/%.1f) banked=%d returned=%d "
