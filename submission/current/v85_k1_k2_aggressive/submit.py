"""V85 K1=0.90 + K2=0.95 aggressive.
K1 gets 0.90 (less than V83 but more conservative GPT-only), K2 gets 0.95 cap.
Same budget structure: K1 (0.90) + K2 (remaining) ≤ 0.90 total.
...
K1_REPLAY_SAFE = 0.90  # Gemma; also GPT fallback when K2 probe fails
# GPT: K1 fill is capped at 0.90 of budget, leaving room for K2 candidates.
# K2 candidates are capped at remaining budget after K1, total ≤ 0.90 * 9000.
K1_GPT_REPLAY_SAFE = 0.90
K2_REPLAY_SAFE_MAX = 0.95
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        # They use the remaining replay budget after K1, capped at 0.95 to avoid
        # variance blow-up, so total K1 + K2 replay cost stays ≤ 0.90 × 9000s.
        ...
        if is_gpt and search_time_left():
            ...
            # Total replay cost (K1 + K2) must be ≤ K1_REPLAY_SAFE * budget (0.90 * 9000 = 8100s).
            # K2's portion is capped at K2_REPLAY_SAFE_MAX * budget (0.95 * 9000 = 8550s).
        ...
        print(
            "[V85] model=%s selected=%s(k%d) score=%.2f cost=%.3fs fill_unit=%.2fs "
