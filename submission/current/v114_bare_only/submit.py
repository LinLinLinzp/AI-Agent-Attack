"""V114 bare-only fastest replay.

Hypothesis: bare template (no special-token) -> model generates fewer
tokens -> faster replay -> more candidates processed in 9000s.

V100 special-token may cause long CoT -> slower replay (~8.78s/cand).
Bare may replay in ~6-7s -> 1286-1500 candidates -> 115-135分.

Risk: bare fire_rate may be lower than special-token.
...
# Bare-only templates: shortest possible, no special-token.
# Hypothesis: bare template → model generates fewer tokens → faster replay
# → replay processes more candidates in 9000s → higher score.
# V100 special-token may cause long CoT → slower replay (~8.78s/cand).
# Bare may replay in ~6-7s → 1286-1500 candidates → 115-135分.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V114] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f(raw=%.2f/%.1f) banked=%d returned=%d "
