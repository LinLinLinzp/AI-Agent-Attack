"""V126 throughput baseline: terminate-only template pool.
...
V126 replaces V100's mixed pool (3 injection + 2 bare) with a pure terminate pool
(5 templates, all <|end|>-terminated, analysis/commentary channels only, NO final
channel, NO bare natural-language). Selection stays raw_rate (V100's method) so the
only variable vs V100 is pool composition. Competition sandbox only."""
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V126] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f(raw=%.2f/%.1f) banked=%d returned=%d "
