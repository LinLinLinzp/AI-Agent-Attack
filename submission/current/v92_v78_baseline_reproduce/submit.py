"""V92 V78 baseline reproduce. K1_REPLAY_SAFE=0.85, K1-only (K2 phase removed).
Reproduces V78's 91.44 baseline without K2 overhead. Competition sandbox only."""
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V92] model=%s selected=%s(k%d) score=%.2f cost=%.3fs fill_unit=%.2fs "
