"""V86 V79 fix - K1=0.95 safe for current environment, K2 opportunistic.
K1 reduced from 0.99 to 0.95 to avoid timeout in current environment.
K2 is purely opportunistic and uses whatever replay budget remains after K1 fill.
If K1 fills right up to 0.95, K2 gets nothing — that is fine.
...
K1_REPLAY_SAFE = 0.95  # Gemma and GPT both: K1 safe for current environment
# GPT: K1 stays at 0.95; K2 is purely opportunistic, using whatever leftover
# budget remains after K1 fill.  Total K1 + K2 replay cost ≤ 0.95 * 9000.
...
# K2 templates: short, minimal prompts (same as V78).
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        # K2 is purely opportunistic: K1 already filled to 0.95, K2 uses whatever
        # leftover replay budget remains after K1.  Total K1 + K2 ≤ 0.95 × 9000s.
        ...
        if is_gpt and search_time_left():
            ...
            # K2 fill: use whatever leftover replay budget remains.
            # Total replay cost (K1 + K2) must be ≤ K1_REPLAY_SAFE * budget (0.95 * 9000 = 8550s).
            # K2 is not capped independently — it just uses whatever is left.
            total_replay_cap = K1_REPLAY_SAFE * budget
            k2_replay_cap = max(0.0, total_replay_cap - replay_cost)
        ...
        print(
            "[V86] model=%s selected=%s(k%d) score=%.2f cost=%.3fs fill_unit=%.2fs "
