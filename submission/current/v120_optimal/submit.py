"""V120 optimal combination — 4 shortest special-token variants.

Combines V116/V117 insights: only use shortest templates (no "Routine.").
4 variants race: commentary (no content) + analysis_short (no content).
Less probe overhead than V118 (4 vs 6), more variants than V116 (4 vs 1).

If any variant replays <8.09s/cand -> 1112+ cands -> 100+ score.
...
# V120: Optimal combination — 4 shortest special-token variants.
# Only commentary (no message content) and short analysis (no "Routine.").
# Race selects fastest-replay variant with highest fire_rate.
# Fewer templates than V118 (4 vs 6) = less probe overhead = more fill time.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V120] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f(raw=%.2f/%.1f) banked=%d returned=%d "
