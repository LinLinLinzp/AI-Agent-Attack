"""V95 double channel collapse + micro-opt. K1_REPLAY_SAFE=0.95, FILL_HOPS=1,
PROBE_REPS=2, RACE_EXTRA_REPS=1, MARGIN_S=20, SLOWEST0=15, 3 templates.
Pre-fills analysis AND final channels. Competition sandbox only."""
...
K1_TEMPLATES: tuple[tuple[str, str, int, int], ...] = (
    ...
    # double_close: collapse BOTH analysis and final channels. Pre-fills the final channel so the model
    # skips wrap-up generation entirely, potentially halving per-candidate cost vs analysis-only collapse.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V95] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f banked=%d returned=%d "
