"""V131 static fill to 2000. Root cause of the ~92 ceiling: V100's fill loop calls
trial() (a real env.interact, ~8.7s of model generation) for EVERY candidate, so the
9000s generation budget only yields ~1025 candidates even though replay can handle
2000 (gateway: replay 0.93-1.04s/cand, 100% fire, cap 2000).

Fix: the template's fire ability is already proven by probe/race (100% fire on real
Kaggle infra). So fill STATICALLY -- emit template+unique-domain messages with no
per-candidate interact. This fills to MAX_CANDIDATES=2000 in near-zero time.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V131] selected=%s k=%d fire_rate=%.3f returned=%d pool=%s slowest=%.2f | %s"
            % (
