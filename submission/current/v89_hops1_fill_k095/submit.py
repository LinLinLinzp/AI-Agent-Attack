"""V89 hops-1 fill, K1_REPLAY_SAFE=0.95. Fill probes at max_tool_hops=1 to skip
scoring-irrelevant wrap-up generation (~1.5-2x faster fill), while replay cost
is sized from the 8-hop probe measurements. Targets 100+ score. Competition sandbox only."""
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V89] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f banked=%d returned=%d "
