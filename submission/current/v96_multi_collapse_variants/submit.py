"""V96 multi-collapse variants. 8 templates: 5 original + 3 new collapse variants
(double_close, final_only, commentary_final). K1_REPLAY_SAFE=0.95, FILL_HOPS=1.
...
K1_TEMPLATES: tuple[tuple[str, str, int, int], ...] = (
    ...
    # commentary_final: use commentary channel instead of analysis + final close. GPT-OSS tool calls
    # are in commentary channel, so pre-filling commentary may directly emit the tool call.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V96] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f banked=%d returned=%d "
