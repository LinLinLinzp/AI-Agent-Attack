"""V155: safety floor -- exact V100 replica (92.25 LB, historical best).

Byte-identical to V100 so the inject-led race->bank->fill framework and its
3-inject + 2-bare template pool serve as the deterministic ~92-point floor under
the experimental V151-V154. Competition sandbox only."""
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V155] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f(raw=%.2f/%.1f) banked=%d returned=%d "
