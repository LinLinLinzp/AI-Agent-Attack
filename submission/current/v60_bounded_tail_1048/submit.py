"""V60 bounded-tail test built on the 88.965 C baseline.
Keeps full-hop exact validation, then appends a small K1-only unvalidated tail
below the fixed-count timeout boundary observed in V58.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        if selected_k == 1 and candidates:
            tail_target = min(int(TAIL_TARGET_K1), MAX_CANDIDATES)
            while len(candidates) < tail_target and tail_added < int(TAIL_EXTRA_MAX):
                current_index = fill_index
                fill_index += 1
                message = _message(selected_template, current_index, selected_k)
                if message in returned_seen:
                    continue
                candidates.append(_candidate(selected_template, current_index, selected_k))
                returned_seen.add(message)
        ...
        print(
            "[v60_bounded_tail_1048] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f banked=%d returned=%d "
            "tail=%d replay_cost=%.0f/%.0f fill=%d/%d slowest=%.2f | %s"
            % (
