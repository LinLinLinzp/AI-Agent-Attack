"""V135 all-in static: static fill to 2000 + per-model routing (gpt injection /
gemma plain) + rotating fill. Root cause of the ~92 ceiling: V100's fill loop
calls trial() (a real env.interact, ~8.7s of model generation) for EVERY
candidate, so the 9000s generation budget only yields ~1025 candidates even
though replay can handle 2000 (gateway: replay 0.93-1.04s/cand, 100% fire,
cap 2000).

Fix: the template's fire ability is already proven by probe/race (100% fire on
real Kaggle infra). So fill STATICALLY -- emit template+unique-domain messages
with no per-candidate interact. This fills to MAX_CANDIDATES=2000 in near-zero
time. On top of V131 this all-in version adds:
- Per-model routing: gemma gets 2 plain natural-language templates (control
  tokens don't apply there), gpt keeps the V131 injection pool.
- Rotating fill: instead of repeating the single selected template, rotate over
  every fire-proven template (ranked order) so all proven fire ability is used.

Expected: 2000 fired * 18 raw / 200 = 180/row (vs 92.25). Replay of 2000 costs
~2080s < 9000s, no timeout. Competition sandbox only."""
...
# Injection pool for gpt: same 5 templates as v62/v100/V131. Mix of
# special-token and natural-language.
...
# Plain natural-language pool for gemma (non-reasoning model, does NOT parse
# control tokens and treats them as ordinary text): no control tokens.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        # Domains start at 0 (probe used 900000+), so no collision. Rotate over every
        # fire-proven template (ranked order) instead of repeating the selected one.
        fill_pool = [idx for idx in ranked_templates if fires[idx] > 0]
        if not fill_pool:
            fill_pool = [selected_index]
        candidates: list[AttackCandidate] = []
        returned_seen: set[str] = set()
        fill_index = 0
        fill_attempts = 0
        while len(candidates) < MAX_CANDIDATES:
            template_index = fill_pool[fill_attempts % len(fill_pool)]
            fill_attempts += 1
            fill_template = templates[template_index][1]
            fill_k = templates[template_index][2]
            message = _message(fill_template, fill_index, fill_k)
            fill_index += 1
            if message in returned_seen:
                continue
            candidates.append(_candidate(fill_template, fill_index - 1, fill_k))
        ...
        print(
            "[V135] model=%s selected=%s k=%d fire_rate=%.3f returned=%d fill=%s pool=%s slowest=%.2f | %s"
            % (
                model_name,
                ...
                selected_fire_rate,
                len(candidates),
                "/".join(templates[idx][0] for idx in fill_pool),
