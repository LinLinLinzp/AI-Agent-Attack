"""V129 throughput baseline: terminate-only template pool + PROBE_REPS 3 -> 5.
...
The wrap-up dominates. Templates ending in <|end|> with a tiny wrap-up make the
model stop immediately after the tool call, shrinking replay time and letting more
candidates fit in 9000s.

V129 keeps V126's pure terminate pool (5 templates, all <|end|>-terminated,
analysis/commentary channels only, NO final channel, NO bare natural-language)
and raises PROBE_REPS from 3 to 5. With only 3 probe samples the template quality
estimate has high variance and the wrong template can be selected; 5 samples lower
the variance so the selection more reliably picks the template with the fastest
replay. Selection stays raw_rate (V100's method). Competition sandbox only."""
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[V129] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f(raw=%.2f/%.1f) banked=%d returned=%d "
