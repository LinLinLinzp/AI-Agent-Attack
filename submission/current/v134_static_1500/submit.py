"""V134 static fill to 1500 (conservative hedge of V131). Root cause of the ~92
...
per-candidate interact. This fills to MAX_CANDIDATES=1500 in near-zero time.

Why 1500 instead of 2000: local replay measured 0.93-1.04s/cand, but if the private
replay is slower (e.g. 4-5s/cand), 2000 candidates could exceed the 9000s replay
budget and get truncated. 1500 leaves headroom (1500*5=7500s < 9000s) while still
far above the ~1025 that interactive fill produced. Conservative choice under
uncertain private replay speed. Competition sandbox only."""
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        # interact. Probe/race already proved the template fires; replay handles 1500.
        ...
        print(
            "[V134] selected=%s k=%d fire_rate=%.3f returned=%d pool=%s slowest=%.2f | %s"
            % (
