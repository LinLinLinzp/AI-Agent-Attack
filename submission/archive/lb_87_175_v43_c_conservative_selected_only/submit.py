"""V43 conservative selected-template replay packing.
Keeps C's K1/K2 template family but returns only the selected lane.
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        for template_index in range(len(templates)):
            ...
            if sample_count < PROBE_REPS or fire_rate < MIN_FIRE_RATE:
                continue
            if templates[template_index][2] > 1 and fire_rate < K2_MIN_FIRE_RATE:
                continue
        ...
        print(
            "[v43_c_conservative_selected_only] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f banked=%d returned=%d "
