"""V63 single-post hop1 clean engine.
Starts from the corrected single-post probe race, then tests the frontier-style
hop1 fill lever with a tighter replay cap to avoid the V57 timeout shape.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[v63_singlepost_hop1_clean] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f banked=%d returned=%d "
            "replay_cost=%.0f/%.0f fill=%d/%d fast=%d audit=%d fail=%d pool=%s slowest=%.2f | %s"
            % (
