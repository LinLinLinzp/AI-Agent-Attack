"""V55 conservative K2 probe-ledger candidate.
Selects the replay-proven K2 inj-list only when current-row probes are exact;
otherwise falls back to K1. Caps are lower than V54 to test a safer raw34 path.
...
K2_SCORE_BIAS = 1.05
LATENCY_MULT = 1.35
K1_LATENCY_FLOOR_S = 7.70
K2_LATENCY_FLOOR_S = 8.50
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        print(
            "[v55_k2_conservative_650] selected=%s k=%d fire=%.2f unit=%.2f returned=%d score_est=%.2f | %s"
