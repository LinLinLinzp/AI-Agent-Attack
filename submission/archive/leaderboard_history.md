...
| 69.545 | unknown | `lb_69_545_v39_gpt_k3_k2_aggressive/` | V39 GPT K3/K2 aggressive | Adds GPT K3 templates and stronger high-K selection bias with gemma K1 fallback | Scored far below C. Treat broad K3/K2 raw-density as unstable until hosted telemetry proves otherwise. |
...
| 87.920 | unknown | `lb_87_920_v59_two_stage_exact_race/` | V59 two-stage exact race | One exact probe across all templates, then extra probes only for the top three | Below the 88.965 baseline. Concentrating probe budget on early leaders did not improve hosted throughput. |
| timeout | n/a | `timeout_v60_bounded_tail_1048_2026_07_28/` | V60 bounded unvalidated tail | Full-hop exact baseline plus a capped K1-only unvalidated tail | Kaggle displayed Submission Format Error; classified as timeout. Even a bounded tail crossed the hosted full-chain safety boundary. |
| 80.505 | unknown | `lb_80_505_v61_compact_k2_exact/` | V61 compact GPT K2 | Shorter GPT-only K2 prompts with strict full-hop validation | Large regression. Prompt shortening did not make K2 replay density transfer reliably. |
| 92.160 | unknown | `lb_92_160_v62_singlepost_confidence_clean/` | V62 K1-only confidence race | Removes GPT K2, keeps exact full-hop validation, races three leaders, `K1_REPLAY_SAFE = 0.99` | Best confirmed result. The gain came from a clean K1 path and better template-selection confidence without weakening validation. |
| 88.020 | unknown | `lb_88_020_v63_singlepost_hop1_clean/` | V63 audited hop1 fill | Adds hop1 fill with periodic full-hop audits and lowers replay safety to `0.975` | Finished but regressed. Cheap validation did not preserve enough effective full-chain success/volume. |
| timeout | n/a | `timeout_v64_lean_k1_race_2026_07_30/` | V64 lean K1 race | V62 with race pool reduced from three leaders to two | Timeout despite a tiny change, proving V62's `0.99` replay cap has little hosted variance margin. |
| timeout | n/a | `timeout_v65_short_k1_microtemplates_2026_07_30/` | V65 short K1 microtemplates | Adds shorter K1 prompt forms to reduce output time | Timeout. Shorter prompts did not yield a safe end-to-end runtime improvement. |
| timeout | n/a | `timeout_v66_strict_k2_return_2026_07_30/` | V66 strict GPT K2 | Enables K2 only after a strict replay-adjusted advantage over K1 | Timeout. Local K2 validation and conservative selection gates did not ensure hosted replay safety. |
| timeout | n/a | `timeout_v67_per_model_prompt_split_2026_07_30/` | V67 model-specific K1 pools | Separate GPT and Gemma K1 prompt pools | Timeout. Model routing added no demonstrated hosted margin; the Gemma chat-template assumption remains unproven. |
| timeout | n/a | `timeout_v68_gpt_k3_relay_probe_safe_2026_07_31/` | V68 GPT-only K3 relay probe | Strict 3/3 full-hop K3 relay gate; Gemma remains K1 | Timeout. Local K3 success did not transfer safely to hosted full-chain replay. |
| timeout | n/a | `timeout_v69_gpt_k3_relay_scale_safe_2026_07_31/` | V69 GPT-only K3 relay scale | V68 with a larger K3 replay fraction | Timeout. Scaling the locally successful K3 relay remains budget-unsafe. |
| timeout | n/a | `timeout_v70_gpt_k2_k3_relay_cascade_safe_2026_07_31/` | V70 GPT K2/K3 relay cascade | Prefer strict K3, then K2, with K1 fallback; Gemma remains K1 | Timeout. The entire relay cascade is rejected as a submission mainline in the current hosted environment. |
...
- `V39` / `lb_69_545_v39_gpt_k3_k2_aggressive`: public LB `69.545`.
