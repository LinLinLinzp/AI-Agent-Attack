"""V59 two-stage exact race built on the 88.965 C baseline.
Runs one exact probe across every template, then spends the remaining probe
budget only on the early leaders before doing the normal exact replay-ledger fill.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def run(self, env: Any, config: AttackRunConfig | None) -> list[AttackCandidate]:
        ...
        def current_raw_rate(template_index: int) -> float:
            ...
            rate = raw_scores[template_index] / total_time
        ...
        print(
            "[v59_two_stage_exact_race] selected=%s k=%d score=%.3f cost=%.3f fill_unit=%.2f banked=%d returned=%d "
