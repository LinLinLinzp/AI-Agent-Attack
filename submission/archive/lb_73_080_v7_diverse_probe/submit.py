...
class AttackAlgorithm(AttackAlgorithmBase):
    """Diverse probe: warmup + probing + 4 rotated templates, REPLAY_SAFE=0.97."""
    ...
    def _fill(self, env: AttackEnvProtocol, budget: float, max_hops: int) -> list[AttackCandidate]:
        ...
        while (
            ...
            message = template[1].format(url=_url(fill_index), payload=PAYLOAD)
        ...
        try:
            ...
            print(
                "[v7_diverse_probe] selected=%s rate=%.3f returned=%d replay_cost=%.0f/%.0f | %s"
